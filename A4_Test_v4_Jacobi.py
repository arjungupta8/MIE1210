...
# ---------------------------------------------------------------------------
# Iterative solver (Jacobi vectorization)
# ---------------------------------------------------------------------------

def solve(phi, phi_star, A_p, A_e, A_w, A_n, A_s, source,
          alpha, epsilon, max_inner_iteration, l2_norm_initial):
    ...
    for _ in range(max_inner_iteration):
        ...
        rhs = (
            A_e[:, :] * np.roll(phi, -1, axis=1) +
            A_w[:, :] * np.roll(phi,  1, axis=1) +
            A_n[:, :] * np.roll(phi, -1, axis=0) +
            A_s[:, :] * np.roll(phi,  1, axis=0) +
            source
        )
        ...
        # under-relaxation
        phi_new = alpha * phi_new + (1 - alpha) * phi_star
...
for n in range(1, max_outer_iteration + 1):
    ...
    u, l2_norm_x = solve(
        ...
        source_x, alpha_uv, epsilon_uv, max_inner_iteration_uv, l2_norm_x
    )

    # Solve v-momentum
    v, l2_norm_y = solve(
        v, v_star, A_p, A_e, A_w, A_n, A_s,
        source_y, alpha_uv, epsilon_uv, max_inner_iteration_uv, l2_norm_y
    )
    ...
    p_prime, l2_norm_p = solve(
        ...
        source_p, dummy_alpha_p, epsilon_p, max_inner_iteration_p, l2_norm_p
    )
