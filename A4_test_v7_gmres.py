...
from scipy.sparse.linalg import gmres, spilu, LinearOperator, bicg
...
def solve(phi, phi_star, A_p, A_e, A_w, A_n, A_s, source,
    ...
    # -------------------------------------------------------------------
    # 1. Flatten unknowns and assemble sparse matrix for BiCG
    # -------------------------------------------------------------------
    N = n_x * n_y
    A = lil_matrix((N, N))
    b = np.zeros(N)
    ...
    def idx(i, j):
        """Convert 2D interior index → 1D index."""
        return (i - 1)*n_x + (j - 1)

    # Build matrix from stencil
    for i in range(1, n_y + 1):
        for j in range(1, n_x + 1):
            ...
            if j < n_x:  # east
                A[row, idx(i, j+1)] = -A_e[i, j]
            if j > 1:    # west
                A[row, idx(i, j-1)] = -A_w[i, j]
            if i < n_y:  # south
                A[row, idx(i+1, j)] = -A_s[i, j]
            if i > 1:    # north
                A[row, idx(i-1, j)] = -A_n[i, j]
    ...
    # 2. Use BiCG to solve
    # -------------------------------------------------------------------
    phi_vec0 = phi[1:n_y+1, 1:n_x+1].flatten()

    phi_vec, info = bicg(A, b, x0=phi_vec0, rtol=epsilon, maxiter=max_inner_iteration)

    if info < 0:
        print("❌ BiCG failed due to an error")
    elif info > 0:
        print("⚠️ BiCG did not converge within max iterations")

    # -------------------------------------------------------------------
    # 3. Write solution back into phi array and apply under-relaxation
    # -------------------------------------------------------------------
    phi_new_full = phi.copy()
    phi_new = phi_vec.reshape((n_y, n_x))

    for i in range(1, n_y + 1):
        for j in range(1, n_x + 1):
            # SIMPLE under-relaxation applied AFTER the BiCG solve
            phi_new_full[i, j] = alpha * phi_new[i-1, j-1] + (1 - alpha) * phi_star[i, j]

    # L2 norm of correction (approximate)
    l2 = np.linalg.norm(phi_vec - phi_vec0)

    return phi_new_full, l2
